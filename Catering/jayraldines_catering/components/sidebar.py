...
class Sidebar(QFrame):
    ...
    def __init__(self):
        ...
        logo_layout.setContentsMargins(20, 24, 12, 20)
        ...
        name_lbl  = QLabel("Owner")
        name_lbl.setStyleSheet("color: #F9FAFB; font-weight: 700; font-size: 13px;")
        email_lbl = QLabel("admin@jayraldines.com")
        email_lbl.setStyleSheet("color: #6B7280; font-size: 11px;")
        self.user_info.addWidget(name_lbl)
        self.user_info.addWidget(email_lbl)
        user_layout.addLayout(self.user_info)
        user_layout.addStretch()

        self.logout_lbl = QLabel()
        self.logout_lbl.setPixmap(
            get_icon("log-out", color="#6B7280", size=QSize(15, 15)).pixmap(QSize(15, 15))
        )
        user_layout.addWidget(self.logout_lbl)
