import os
from PySide6.QtGui import QIcon, QPixmap, QPainter, QColor
from PySide6.QtCore import Qt, QSize
from PySide6.QtSvg import QSvgRenderer
from PySide6.QtWidgets import QApplication

_ICONS_DIR = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "assets", "icons", "svg")

ICON_MAP = {
    "dashboard":    "dashboard.svg",
    "bookings":     "bookings.svg",
    "calendar":     "calendar.svg",
    "reports":      "reports.svg",
    "plus":         "plus.svg",
    "export":       "export.svg",
    "filter":       "filter.svg",
    "trash":        "trash.svg",
    "check":        "check.svg",
    "chevron-left":  "chevron-left.svg",
    "chevron-right": "chevron-right.svg",
    "close":        "close.svg",
    "log-out":      "log-out.svg",
    "date-range":   "date-range.svg",
    "reset-zoom":   "reset-zoom.svg",
    "eye":          "eye.svg",
}

DEFAULT_SIZE = QSize(20, 20)
DEFAULT_COLOR = "#9aa0a6"


def get_icon(name: str, color: str = DEFAULT_COLOR, size: QSize = DEFAULT_SIZE) -> QIcon:
    svg_file = ICON_MAP.get(name)
    if not svg_file:
        return QIcon()

    svg_path = os.path.join(_ICONS_DIR, svg_file)
    if not os.path.exists(svg_path):
        return QIcon()

    with open(svg_path, "r", encoding="utf-8") as f:
        svg_data = f.read()

    svg_data = svg_data.replace('stroke="currentColor"', f'stroke="{color}"')
    svg_bytes = svg_data.encode("utf-8")

    renderer = QSvgRenderer(svg_bytes)
    pixmap = QPixmap(size)
    pixmap.fill(Qt.transparent)

    painter = QPainter(pixmap)
    painter.setRenderHint(QPainter.Antialiasing)
    renderer.render(painter)
    painter.end()

    icon = QIcon()
    icon.addPixmap(pixmap, QIcon.Normal)

    hi_dpi_pixmap = QPixmap(QSize(size.width() * 2, size.height() * 2))
    hi_dpi_pixmap.fill(Qt.transparent)
    painter2 = QPainter(hi_dpi_pixmap)
    painter2.setRenderHint(QPainter.Antialiasing)
    renderer.render(painter2)
    painter2.end()
    hi_dpi_pixmap.setDevicePixelRatio(2.0)
    icon.addPixmap(hi_dpi_pixmap, QIcon.Normal)

    return icon


def nav_icon(name: str) -> QIcon:
    return get_icon(name, color="#9aa0a6", size=QSize(18, 18))


def nav_icon_active(name: str) -> QIcon:
    return get_icon(name, color="#c5a46d", size=QSize(18, 18))


def btn_icon_primary(name: str) -> QIcon:
    return get_icon(name, color="#0f1115", size=QSize(16, 16))


def btn_icon_secondary(name: str) -> QIcon:
    return get_icon(name, color="#eaeaea", size=QSize(16, 16))


def btn_icon_muted(name: str) -> QIcon:
    return get_icon(name, color="#9aa0a6", size=QSize(16, 16))
