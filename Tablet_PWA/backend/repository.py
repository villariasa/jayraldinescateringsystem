"""
Business logic layer — Customer, Package/Menu, Order, Billing, Terms.

Ported 1:1 from the original PySide6 Tablet app's utils/repository.py so
the PWA replicates the exact same order-creation semantics (booking ref
format, invoice/payment record creation, status derivation, etc). See
Tablet/utils/repository.py for the reference implementation this was
ported from.
"""
...
_BOOKING_REF_PREFIX = "TB"  # Same prefix as the original tablet app, distinct
                            # from the PC app's "BK-" prefix.
...
def add_customer(name: str, contact: str = "", email: str = "", address: str = "") -> int:
    ...
    return db.execute(
        ...
        (name, contact_clean, (email or "").strip(), (address or "").strip()),
    )
...
def update_customer(customer_id: int, name: str, contact: str = "", email: str = "", address: str = "") -> bool:
    ...
    db.execute(
        ...
        (name, (contact or "").strip(), (email or "").strip(), (address or "").strip(), customer_id),
    )
    return True
...
def get_package_menu_choices() -> dict[str, list[dict]]:
    ...
    for r in all_items:
        ...
        grouped.setdefault(cat, []).append({
            ...
            "description": r["mi_description"] or "",
        })
    return grouped
...
def create_order(order: dict) -> dict:
    ...
    for c in order.get("additional_charges", []):
        db.execute("""
            ...
            VALUES (?, ?, ?, ?, ?)
        """, (booking_id, c["description"], float(c["amount"]), _date.today().isoformat(), order.get("actor", "Tablet PWA")))
    ...
    if down_payment > 0:
        db.execute("""
            ...
            VALUES (?, ?, ?, ?, ?, 1)
        """, (invoice_id, down_payment, _date.today().isoformat(), order.get("payment_method", "Cash"), "Down payment recorded via kiosk PWA"))

    if order.get("terms_version"):
        record_terms_acknowledgement(booking_id, order["terms_version"], order["customer_name"])
...
def get_all_orders(limit: int = 200) -> list[dict]:
    rows = db.fetchall("""
    ...
    return [
        {
            ...
            "total": float(r["bk_total_amount"] or 0.0),
