...
SELECT
    ...
    COALESCE((SELECT COUNT(*) FROM bookings WHERE DATE(event_date) = CURRENT_DATE AND status IN ('CONFIRMED', 'COMPLETED')), 0)::INT AS today_bookings,
...
DECLARE
    ...
    v_booking_id  INT;
BEGIN
    SELECT total_amount, amount_paid, booking_id INTO v_total, v_paid, v_booking_id
    FROM invoices WHERE id = p_invoice_id FOR UPDATE;
    ...
    UPDATE bookings
    SET amount_paid = p_new_paid, updated_at = NOW()
