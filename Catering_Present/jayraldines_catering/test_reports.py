...
sample data. Run directly (``python test_reports.py``) to preview the layout
...
def apply_fade_in(widget, duration=800):
    ...
    # Attached to widget to prevent Segfaults (keeps a Python ref so Qt won't
    # delete the effect/animation mid-run).
