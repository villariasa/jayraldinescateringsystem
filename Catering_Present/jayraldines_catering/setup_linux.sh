...
address_tables_ok() {
    PGPASSWORD="$PG_PASS" "$PSQL" -U "$PG_USER" -h localhost -p "$PG_PORT" -d "$DB_NAME" \
        -tAc "SELECT 1 FROM information_schema.columns
              WHERE table_name='address_regions' AND column_name='ar_id'
              LIMIT 1;" 2>/dev/null || echo ""
}

# ── Helper: drop broken address tables so the main schema can rebuild them ────
drop_broken_address_tables() {
    info "Dropping broken address tables (wrong column names — will rebuild from main schema)..."
    PGPASSWORD="$PG_PASS" "$PSQL" -U "$PG_USER" -h localhost -p "$PG_PORT" -d "$DB_NAME" << 'DROPSQL'
DROP TABLE IF EXISTS addresses         CASCADE;
DROP TABLE IF EXISTS address_barangays CASCADE;
DROP TABLE IF EXISTS address_cities    CASCADE;
DROP TABLE IF EXISTS address_provinces CASCADE;
DROP TABLE IF EXISTS address_regions   CASCADE;
DROPSQL
    ok "Broken address tables dropped"
}
...
if [ "$DB_EXISTS" = "1" ]; then
    ...
    if [[ "$CHOICE" != "y" && "$CHOICE" != "Y" ]]; then
        ...
        # ── Fix broken address tables if they have the wrong schema ──────────
        ADDR_EXISTS=$("$PSQL" -U "$PG_USER" -h localhost -p "$PG_PORT" -d "$DB_NAME" \
            -tAc "SELECT 1 FROM information_schema.tables WHERE table_name='address_regions';" 2>/dev/null || echo "")
...
ALTER TABLE customers ADD COLUMN IF NOT EXISTS cus_address_id INT REFERENCES addresses(addr_id);
INSERT INTO address_regions (ar_name) VALUES ('Region VII - Central Visayas') ON CONFLICT DO NOTHING;
INSERT INTO address_provinces (ap_region_id, ap_name)
    SELECT ar_id, 'Cebu' FROM address_regions WHERE ar_name = 'Region VII - Central Visayas'
...
"
            ...
            skip "Address tables already correct (or absent — will be created on fresh install)"
        fi
