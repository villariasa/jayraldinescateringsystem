"""Debug helper: inspect the ``sp_update_package`` stored procedure.
...
cur = conn.cursor()
# List the parameters of every sp_update_package* overload, in declaration order.
cur.execute("""
    SELECT specific_name, parameter_name, data_type, parameter_mode, ordinal_position
    FROM information_schema.parameters
    WHERE specific_name LIKE 'sp_update_package%'
...
cur.execute("""
    ...
    WHERE routine_name = 'sp_update_package';
""")
for r in cur.fetchall():
    print("DEF:", r)
